...
//	var $ids = $("#listTable input[name='ids']:enabled");
	...
	var $pageNumberInput = $("#pageNumberInput");
...
/*	// 刷新
	...
	$enabledIds.click( function() {
		var $this = $(this);
		if ($this.prop("checked")) {
			$deleteButton.attr("disabled", false);
			if ($enabledIds.filter(":not(:checked)").size() <= 0) {
...
/*		if (!/^\d*[1-9]\d*$/.test($pageNumber.val())) {
	...
	$submitButton.click (function() {
		$pageNumber.val($pageNumberInput.val());
	});
