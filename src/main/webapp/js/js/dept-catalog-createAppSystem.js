...
//是否为空值
function createAppCheckIsEmpty(obj, name, message) {
    var string = obj.val();
    if(isEmpty(string)) {
...
$("#appSystemDesc").blur(function () {
    createAppCheckIsEmpty($("#appSystemDesc"), 'AppSystemDesc', "请输入应用系统说明，1000字以内");
});
$("#contractorName").blur(function () {
    createAppCheckIsEmpty($("#contractorName"), 'ContractorName', "请输入承建单位名称，100字以内");
});
...
$("#databaseProductDesc").blur(function () {
    createAppCheckIsEmpty($("#databaseProductDesc"), 'DatabaseProductDesc', "请输入数据库产品描述，400字以内");
});
$("#accessAddress").blur(function () {
    createAppCheckIsEmpty($("#accessAddress"),'AccessAddress', "请输入应用系统访问地址");
});
...
function createAppSystemCheckAll() {
    ...
    createAppCheckIsEmpty($("#appSystemDesc"), 'AppSystemDesc', "请输入应用系统说明，1000字以内");
    createAppCheckIsEmpty($("#contractorName"), 'ContractorName', "请输入承建单位名称，100字以内");
    createAppCheckIsEmpty($("#onlineDate"), 'OnlineDate', "请选择上线时间");

    createAppCheckIsEmpty($("#databaseTypeCodes"), 'DatabaseTypeCodes', "请选择数据库类型代码");
    createAppCheckIsEmpty($("#databaseProductDesc"), 'DatabaseProductDesc', "请输入数据库产品描述，400字以内");
    createAppCheckIsEmpty($("#accessAddress"), 'AccessAddress', "请输入应用系统访问地址");
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result == "success") {
                    location.href = "/dmall/appsystems";
                } else {
                    dmallError(data.result);
                }
            },
            error: function () {
                dmallError("应用系统信息有误, 保存失败");
                $("#btn_commitCreateInfo").attr('disabled', false);
            }
        })
    }
