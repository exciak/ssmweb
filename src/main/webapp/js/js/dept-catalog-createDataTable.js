/**
 * Created by yi on 2017/4/5.
 */
//检查数据表编号标志
var dataTableDefaultFlag = false;
...
var dataTableArr = [];
var modifyArr = [];
var dataTablePageSize = 5;
var dataTableTotalPages;
...
$(document).ready(function(){
    ...
    $("#dataTable-btn-submit").click(function () {
        ...
        if (dataTableDefaultFlag) {
            ...
            if (dataTableArr.length % dataTablePageSize == 0) {
                dataTableTotalPages = Math.floor(dataTableArr.length / dataTablePageSize);
            } else {
                dataTableTotalPages = Math.floor(dataTableArr.length / dataTablePageSize) + 1;
            }
            if (dataTableArr.length != 1 && dataTableArr.length % dataTablePageSize == 1) {
                initPage(dataTableTotalPages + 1, "create");
                $("#listTable tbody tr").hide();
                catalogTurnToPage(dataTableTotalPages);
            } else {
                catalogTurnToPage(dataTableTotalPages);
            }
        }
    ...
    $("#listTable tbody ").on("click","tr td.deleteBtn a",function (e) {
        e.preventDefault();
        var targetNameIndex = parseInt(($("#pageTag li.active a").text()-1)*dataTablePageSize);
        var targetName = parseInt($(this).parent().parent().index()+targetNameIndex);
        var target = $(this).parent().parent();
        target.remove();

        var obj = dataTableArr[targetName];
        console.log(obj);
        if (null != obj.id) {
        ...
        if(dataTableArr.length == 0){
            dataTableTotalPages = 1;
        }else if (dataTableArr.length % dataTablePageSize == 0) {
            dataTableTotalPages = Math.floor(dataTableArr.length / dataTablePageSize);
        } else {
            dataTableTotalPages = Math.floor(dataTableArr.length / dataTablePageSize) + 1;
        }
        if(parseInt($("#pageTag li.active a").text()) > dataTableTotalPages){
            initPage(dataTableTotalPages);
            catalogTurnToPage(dataTableTotalPages);
        }else {
            var n= parseInt($("#pageTag li.active a").text());
            initPage(n);
            catalogTurnToPage(n);
        }
    });
    ...
    $("#pageTag").on("click",".pageTag",function(){
        if(!$(this).hasClass("disabled")){
            ...
            if(pageNumber != "<" && pageNumber != ">" && pageNumber!= '...'){
                catalogTurnToPage(parseInt(pageNumber));
            }else if(pageNumber == "<"){
                catalogTurnToPage(parseInt(active-1));
            }else if (pageNumber == ">"){
                catalogTurnToPage(parseInt(active+1));
            }
        }
...
function catalogTurnToPage(pageNumber){
    var arr1 = dataTableArr.slice((pageNumber-1)*dataTablePageSize,pageNumber*dataTablePageSize);
    ...
    for(var j=0;j<arr1.length;j++){
        ...
        $("#listTable tbody").append('<tr style="display:table-row">'+
            '<td>'+arr1[j].itemNo+'</td>'+
            '<td class="longText" title="'+arr1[j].itemName+'">'+arr1[j].itemName+'</td>'+
            '<td>'+dataItemFormatCode+'</td>'+
            '<td><a href="javascript:void(0)" onclick="catalogViewMeta(this)">'+arr1[j].metadataCode.split(" ")[0]+'</td>'+
            '<td>'+itemLength+'</td>'+
            '<td class="longText" title="'+arr1[j].itemDesc+'">'+arr1[j].itemDesc+'</td>'+
            ...
            '<td hidden="true">'+arr1[j].lhDicGroupCode+'</td>'+
...
function initPage(pageNumber,type) {
    ...
    if( item.length == 0){
        dataTableTotalPages = 1;
    }else if(item.length%dataTablePageSize == 0){
        dataTableTotalPages = Math.floor(item.length/dataTablePageSize);
    }else{
        dataTableTotalPages = Math.floor(item.length/dataTablePageSize)+1;
    }
    ...
    if(pageNumber == 1){
        ...
        if(pageNumber >= 3 && dataTableTotalPages >= 5){
    ...
    if((dataTableTotalPages - pageNumber) < 2 && pageNumber > 2){
        if((dataTableTotalPages == pageNumber) && pageNumber > 3){
    ...
    if((dataTableTotalPages - pageNumber) >= 1){
        $("#pageTag").append('<li class="pageTag"><a>'+(parseInt(pageNumber)+1)+'</a></li>');
    }
    if(pageNumber <3){
        if((pageNumber + 2) < dataTableTotalPages){
            $("#pageTag").append('<li class="pageTag"><a>'+(parseInt(pageNumber)+2)+'</a></li>');
        }
        if((pageNumber < 2)&& ((pageNumber + 3) < dataTableTotalPages) ){
            $("#pageTag").append('<li class="pageTag"><a>'+(parseInt(pageNumber)+3)+'</a></li>');
        }
    }
    if((dataTableTotalPages - pageNumber) >= 3 && dataTableTotalPages > 5){
        $("#pageTag").append('<li class="pageTag"><a >...</a></li>');
    }

    if(pageNumber == dataTableTotalPages || type == "create"){
        $("#pageTag").append('<li class="disabled pageTag"><a>&gt;</a></li>');
    }else{
        if((dataTableTotalPages - pageNumber) >= 2){
            $("#pageTag").append('<li class="pageTag"><a>'+dataTableTotalPages+'</a></li>');
        }
        $("#pageTag").append('<li class="pageTag"><a>&gt;</a></li>');
    }
}
...
$("#listTable tbody").on("click","tr td.td-modify a",function () {
    ...
    $("#modifyName").val(dataItemTr[1].innerHTML);
    ...
    $("#modifyDescription").val(dataItemTr[5].innerHTML);
    ...
    if(dataItemTr[6].innerHTML == 'true'){
        ...
        $("#modifyLhDicGroupCode").val(dataItemTr[12].innerHTML);
    } else {
...
$("#modify-submit").click(function () {
    ...
    if (dataTableDefaultFlag) {
        ...
        $selector.parent().parent().children("td").eq(1).text($("#modifyName").val());
        ...
        $selector.parent().parent().children("td").eq(5).text($("#modifyDescription").val());
        ...
        $selector.parent().parent().children("td").eq(12).text($("#modifyLhDicGroupCode").val());

        var modifyNameIndex = parseInt(($("#pageTag li.active a").text()-1)*dataTablePageSize)
        ...
        catalogTurnToPage( parseInt($("#pageTag li.active a").text()));
    }
...
function modifyItemCheck() {
    ...
    createCheckIsEmpty($("#modifyDataContent"), 19, "请选择数据元内容标识符");
    checkItemNo($("#modifyNo"), 17, modifyArr);
}

function checkModifyItemName() {
    if (!inputCheckName($("#modifyName").val())) {
        $("#modifyName").addClass("border-red");
        $('.error18').html("*请输入应用系统名称，由中文、字母、数字、下划线组成，1-120个字符");
...
function checkDataTableItem() {
    ...
    createCheckIsEmpty($("#inputDataContent"), 12, "请选择数据元内容标识符");
}
...
function catalogViewMeta(obj){
    $(obj).removeAttr('onclick');
    var metacode = $(obj).text();
    // var catecode = $.trim( $(obj).parent().next().text() );
    $("#catalogViewMetaModal").data("metacode",metacode);
    // $("#catalogViewMetaModal").data("catecode",catecode);
    $.get("/" + prjName + "/metadata/detail",
        {
            metaCode:metacode
            // cateCode:catecode
        },function(data,status){
            $(obj).attr('onclick', 'catalogViewMeta(this);');
            if(data.result != "success"){
                $("#catalogViewMetaModal").modal("hide");
                dmallError(data.result);
            }else{
                ...
                if(meta_data.message != "success"){
                    ...
                    $("#catalogViewMetaModal").modal({backdrop:'static', keyboard: false});
                    ...
                    for(var i in metadata) {
                        ...
                        for (var j in detail) {
                            var qName = '';
                            if(i == 'categories'){
                                $("#catalogViewMetaModal").data("catecode",j);
                                qName = data.qname;
                            }else {
                                qName = j;
                            }
                        ...
                        if (i == "表示类") {
                            ...
                            htmlStr += '<th class="text-left">值域:</th><td class="text-left">' +
                                '<a href="javascript:void(0)" onclick="catalogViewValueRange()" class="fa fa-file-text-o text-primary oper-icon-2"></a></td><td style="border:0"></td>';
                            if (num % 3 == 2) {
                                htmlStr += '</tr>';
                            }
                            num++;
...
function catalogViewValueRange(){
    $('#catalogViewMetaModal').modal('hide');
    $('#catalogViewValueRangeModal').modal('hide');
    $('#catalogViewValueRangeModal').modal({backdrop: 'static', keyboard: false});
    $("#valueRangeTable tr:not(:first)").remove();
    var metacode = $("#catalogViewMetaModal").data("metacode");
    var catecode = $("#catalogViewMetaModal").data("catecode");
...
function catalog_modal_close(){
    $('#catalogViewValueRangeModal').hide();
    $('#catalogViewValueRangeModal').modal('hide');
    $('#catalogViewMetaModal').modal({backdrop: 'static', keyboard: false});
}
function first_catalog_modal_close(){
    $('#catalogViewMetaModal').modal('hide');
}

var type = "";
$("#dataTablePreviousPage").click(function () {
    if (parseInt($("#dataTableCurrentPage").text()) > 1) {
        switch (type){
            case "dicDataTableNo":
                getDicDataTableNo(parseInt($("#dataTableCurrentPage").text()) - 1, $('#dataTableAppSystemId').val());
                break;
            case "dicCodeItemNo":
                getDicCodeItemNo(parseInt($("#dataTableCurrentPage").text()) - 1, $('#inputDicDataTableNo').val());
                break;
            case "dicDescItemNo":
                getDicCodeItemNo(parseInt($("#dataTableCurrentPage").text()) - 1, $('#inputDicDataTableNo').val());
                break;
            ...
            case "modifyDicCodeItemNo":
                getDicCodeItemNo(parseInt($("#dataTableCurrentPage").text()) - 1, $('#modifyDicDataTableNo').val());
                break;
...
$("#dataTableNextPage").click(function () {
    if (parseInt($("#dataTableCurrentPage").text()) < parseInt($("#dataTableTotalPage").text())) {
        switch (type){
            case "dicDataTableNo":
                getDicDataTableNo(parseInt($("#dataTableCurrentPage").text()) + 1, $('#dataTableAppSystemId').val());
                break;
            ...
            case "lhDicItemNo":
                ...
                break;
            case "modifyDicDataTableNo":
                getModifyDicDataTableNo(parseInt($("#dataTableCurrentPage").text()) + 1, $('#dataTableAppSystemId').val());
                break;
            case "modifyDicCodeItemNo":
                getModifyDicCodeItemNo(parseInt($("#dataTableCurrentPage").text()) + 1, $('#modifyDicDataTableNo').val());
                break;
            case "modifyDicDescItemNo":
                getModifyDicCodeItemNo(parseInt($("#dataTableCurrentPage").text()) + 1, $('#modifyDicDataTableNo').val());
                break;
    ...
    $.get(rootPath + "/appsystems/datatables/getPage", {
        ...
        function (data, status) {
            if ((status == "success") && (data.result == "success")) {
                ...
                $("#dataTableTotalPage").html(data.totalPage);
                if (data.totalPage == 0) {
                    $("#dataTableCurrentPage").html(0);
                } else {
                    $("#dataTableCurrentPage").html(data.pageNumber);
                }
            } else {
